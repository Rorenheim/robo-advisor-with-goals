...
class PortfolioOptimizerPast:
    '''
    A modified version of PortfolioOptimizer for backtesting with historical data.
    '''
    ...
    def __init__(self,
                 ...
                 max_pos=1.0,
                 min_pos=0.0,
                 end_date=None):  # Added end_date parameter
        ...
        self.end_date_ = end_date if end_date else datetime.datetime.today().strftime('%Y-%m-%d')
        self._fetch_data()
    ...
    def _fetch_data(self):
        ...
        for asset in self.asset_basket_:
            try:
                temp = yf.download(asset, start="2000-01-01", end=self.end_date_)
                if not temp.empty:
                    temp = temp[['Adj Close']]
