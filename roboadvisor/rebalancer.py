...
class RebalancingSimulator:
    ...
    def _data_prep(self):
        '''
        Prepares data required for the simulation from the portfolio object.  Creates a list for each asset 
    ...
    def _initialize_portfolio(self):
        ...
        for i in range(len(starting_vals)):
            ...
            if self.frac_units_:
                ...
                self.starting_residual_cash -= cash_used
    ...
    def _random_walks(self, plot=False):
        ...
        we're going to create random walks to simulate returns for each asset
        in the portfolio. 
        '''
        starting_vals = self.starting_vals_
        random_walks = []
    ...
    def run_simulation(self, plot=False):
        ...
        print('-----------------')
        print(f'Rebalancing simulation finished in: {time.time() - start:.2f} seconds')
        print(f'Total number of trades executed: {self.total_trades_}')
        print(f'Cost per trade: {round(self.trade_cost_, 2)}')
        print(f'Total trading costs: {round(self.total_trade_cost_, 2)}')
        print(f'Maximum cash balance: {round(max(self.sim_cash_history_), 2)}')
        print(f'Minimum cash balance: {round(min(self.sim_cash_history_), 2)}')
        print(f'Average cash balance: {round((sum(self.sim_cash_history_) / len(self.sim_cash_history_)), 2)}')
        print(f'Fractional units allowed? {str(self.frac_units_)}')
        print('')
        
        for i in range(len(self.asset_list_)):
            weight_history = [x[i] for x in self.sim_weight_vals_]
            asset = self.asset_list_[i]
            print(f'Weight metrics for: {asset}')
            print('-------------------------')
            print(f'  Target portfolio weight: {self.target_weights_[i]}')
            print(f'  Standard deviation of {asset} portfolio weight: {round(statistics.stdev(weight_history), 4)}')
            print(f'  Maximum weight reached for {asset}: {round(max(weight_history), 4)}')
            print(f'  Minimum weight reached for {asset}: {round(min(weight_history), 4)}')
            print(f'  Average of {asset} portfolio weight: {round(statistics.mean(weight_history), 4)}')
            print('')
    ...
    def reset_sim(self):
        '''
        Resets the simulation values back to the original state
        '''
    ...
    def _calculate_sim_metrics(self):
        ...
        print('--------------------------------')
        print(f'Simulations run: {n_samples}')
        print(f'Mean Terminal Portfolio Value: {self.term_val_mean_}')
        print(f'Standard Deviation of Terminal Portfolio Values: {self.term_val_std_}')
        print(f'Mean number of total trades executed: {self.trade_sum_mean_}')
        print(f'Standard Deviation of total trades executed: {self.trade_sum_std_}')
        print(f'Mean total trade cost: {self.sim_trade_cost_mean_}')
        print(f'Standard Deviation of total trade costs: {self.sim_trade_cost_std_}')
        print(f'Duration of each simulation (in years): {round((self.random_walk_iters_ / 252), 2)}')
        print(f'Upper rebalance threshold: {self.thresh_high_}')
        print(f'Lower rebalance threshold: {self.thresh_low_}')
        print('')
            ...
            if weight_diffs[i] > thresh_high:
                ...
                if frac_units == False:
                    if ((target_sell - trade_cost) // unit_prices[i]) >= 1:
                        ...
                        unit_holdings[i] -= allowable_units
                        self.sim_cash_balance_ += allowable_units * unit_prices[i]
                        trade_count += 1
    
                elif frac_units == True:
                    allowable_units = (target_sell - trade_cost) / unit_prices[i]
                    unit_holdings[i] -= allowable_units
                    self.sim_cash_balance_ += (allowable_units * unit_prices[i]) - trade_cost
                    trade_count += 1
                ...
                if frac_units == False:
                    ...
                    if self.sim_cash_balance_ >= (((target_buy - trade_cost) // unit_prices[i]) * unit_prices[i]) - (trade_cost + 1):
                        unit_holdings[i] += (target_buy - trade_cost) // unit_prices[i]
                        self.sim_cash_balance_ -= (((target_buy - trade_cost) // unit_prices[i]) * unit_prices[i]) + trade_cost
                        trade_count += 1
